...
const static char* TAG = "GVRN";

extern SettingsData settings;

TaskHandle_t governorHandle;
static QueueHandle_t event_queue = NULL;
static TimerHandle_t scan_timer = NULL;
static const uint8_t eventQueueLen = 3;

void SendGovernorEvent(const governorEvent event) { xQueueSend(event_queue, &event, 0); }

void scanTmrCallback() {
    governorEvent event = SCAN;
    SendGovernorEvent(event);
}
...
static void HandleEvent(const governorEvent event) {
    switch (event) {
    case SCAN:
        float currentTemp;
...
void GovernorTask(void* pvParameters) {
    ...
    event_queue = xQueueCreate(eventQueueLen, sizeof(governorEvent));
