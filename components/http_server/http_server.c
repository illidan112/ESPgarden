...
static bool http_serverState = false;

bool isWebServerRunning() { return http_serverState; }
void setWebServerState(bool flag) { http_serverState = flag; }
...
static void HandleEvent(const serverEvent event) {
    ...
    case RECONNECT:
        if (!http_serverState) {
            ...
            // app_events_post(APP_EVENT_WIFI_RECONNECT_REQUEST, NULL, 0, portMAX_DELAY);
        }
        break;
    }
}
